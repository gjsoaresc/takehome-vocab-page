...
- **The app had no desktop presentation, reported by the user.** The Aurea design is specified at 390px and nowhere else, so on a laptop the 448px column sat in a wide field of empty paper and read as broken. Rather than invent a second breakpoint the design never had, `#root` becomes a device frame above 880x720: centred, fixed size, rounded, elevated, on a tinted page. `transform: translate(0)` makes it the containing block for every `position: fixed` descendant, so the nav, reward sheets, toasts and the quiz feedback bar pin to the device with no change to any of the six screens - the frame itself must not scroll, because a fixed element inside a transformed *scrolling* ancestor scrolls away with the content, so `main` is the scroller. The three places that measured `100dvh` now read a `--app-h` variable the frame overrides. Verified: six routes x light/dark at 1440x900 and 1280x800 - no frame overflow, no page scroll behind it, nav pinned, both fixed overlays contained; and phone (390) and tablet (768) controls are byte-identical to the previous behaviour.
- **The `:8080` docker image was serving the pre-Aurea bundle** - it was built before the rebuild landed, and the README's Quickstart points reviewers straight at it. Rebuilt; `docker compose up --build` in the README already covers a clean clone.
